...
public class ComponentEditsContextMenu
{
    ...
    /// <summary>
    /// Adds The needed colliders with triggers to allow the object (and its children) to be transparent swapped by the camera at run time.
    /// This gives everything with a Mesh Renderer a Mesh Collider set to convex and trigger as well as a Material Swap script.
    /// It will not edit existing mesh colliders however. If a new one is added it will be convex and trigger.
    /// </summary>
    [MenuItem("GameObject/ComponentEdits/ProjectSpecific/Give Transparent Functionality", false, 0)]
    public static void GiveTransparentFuncionality()
    {
        foreach (GameObject gameObject in Selection.gameObjects)
        {
            List<Transform> allTransforms = gameObject.GetComponentsInChildren<Transform>().ToList();
            foreach (Transform tForm in allTransforms)
            {
                ...
                {
                    tForm.gameObject.AddComponent(typeof(MeshCollider));
                    tForm.GetComponent<MeshCollider>().convex = true;
                    tForm.GetComponent<MeshCollider>().isTrigger = true;
                }
                if (tForm.GetComponent<MeshRenderer>() && tForm.GetComponent<MeshCollider>() && tForm.GetComponent<MaterialSwap>() == null)
                {
                    tForm.gameObject.AddComponent<MaterialSwap>();
                }
                tForm.gameObject.layer = 7; // set to the environment layer
            }
            UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(gameObject.scene);
        }
    }
